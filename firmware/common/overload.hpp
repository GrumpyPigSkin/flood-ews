...
namespace tl {
namespace detail {
template <class... Ts> struct overloader;

template <> struct overloader<> {};

template <class T> struct overloader<T> : T {
  using T::operator();

  template <class U> overloader(U &&u) : T{std::forward<U>(u)} {}
};
...
template <class... Ts> auto overload(Ts &&...ts) {
  return detail::overloader<typename std::decay<Ts>::type...>{
      std::forward<Ts>(ts)...};
}
} // namespace tl
