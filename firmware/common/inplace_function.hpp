...
{
  // CrashReport.BreadCrumbString = "call of uninitialized function";  //
...
#define SG14_INPLACE_FUNCTION_THROW(x)                                         \
  ((common::panic())) // quick hack, make it crash instead of an exception>
...
template <size_t Cap> union aligned_storage_helper {
  struct double1 {
    double a;
  };
  struct double4 {
...
template <size_t Cap, size_t Align = alignof(aligned_storage_helper<Cap>)>
using aligned_storage_t = aligned_storage<Cap, Align>;
...
static_assert(sizeof(std::aligned_storage_t<sizeof(void *)>) == sizeof(void *),
              "C");
static_assert(alignof(std::aligned_storage_t<sizeof(void *)>) ==
                  alignof(void *),
              "D");
#endif
...
public:
  ...
  inplace_function(const inplace_function<R(Args...), Cap, Align> &other)
      : inplace_function(other.vtable_ptr_, other.vtable_ptr_->copy_ptr,
                         std::addressof(other.storage_)) {
    static_assert(
        inplace_function_detail::is_valid_inplace_dst<Capacity, Alignment, Cap,
                                                      Align>::value,
  ...
  inplace_function(inplace_function<R(Args...), Cap, Align> &&other) noexcept
      : inplace_function(other.vtable_ptr_, other.vtable_ptr_->relocate_ptr,
                         std::addressof(other.storage_)) {
    static_assert(
        inplace_function_detail::is_valid_inplace_dst<Capacity, Alignment, Cap,
                                                      Align>::value,
