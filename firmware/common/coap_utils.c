...
static int coap_req_send(const coap_addr_t addr, const char *uri,
  ...
  if (addr.is_str) {
    otIp6AddressFromString(addr.u.str, &msg_info.mPeerAddr);
  } else {
    msg_info.mPeerAddr = addr.u.addr;
  }
  ...
  otCoapMessageInit(msg, OT_COAP_TYPE_NON_CONFIRMABLE, code);
...
int coap_put_req_send(const coap_addr_t addr, const char *uri,
                      uint8_t const *const buf, int len,
                      otCoapResponseHandler handler, void *ctx) {
  return coap_req_send(addr, uri, buf, len, handler, ctx, OT_COAP_CODE_PUT);
}

int coap_get_req_send(const coap_addr_t addr, const char *uri,
                      uint8_t const *buf, int len,
                      otCoapResponseHandler handler, void *ctx) {
...
int coap_resp_send(otMessage *req, const otMessageInfo *req_info,
                   uint8_t const *buf, const int len) {
  otInstance *ot;
  otMessage *resp;
